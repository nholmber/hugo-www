...
In my previous [post]({{< ref "mpi_read.md" >}}), I discussed the benefits of using the message passing interface (MPI) to parse large data files in parallel over multiple processors. In today's post, I will continue exploring how I/O operations can be accelerated in MPI parallelized environments by implementing the corresponding writer routine to the problem described previously. The focus of this post will, however, be on fine tuning MPI I/O performance with environment variables. The second main topic I will discuss is the integration of solid-state drives into high-performance computing systems.
...
Some additional control settings become available when Burst Buffer SSD nodes are used. The most important settings is the number Burst Buffer nodes, which controls how many physical SSDs the output file is striped over. This setting is equivalent to the hint `string_factor` on Lustre. All Burst Buffer settings are controlled through DataWarp. To change the number Burst Buffer nodes, we must first find out the size of each SSD:
...
The output of the above command shows that each SSD is 174416 MB or roughly 200 GB. The number of allocated Burst Buffer nodes can then be changed by requesting SSD capacity that is in between adjacent integer multiples of the SSD size. To clarify, the command below, for example, would request 2 Burst Buffer nodes because `3*174416 MB > 450 GB > 2*174416 MB`.

`#DW jobdw type=scratch access_mode=striped capacity=450GB`

It turns out that changing the number of Burst Buffer nodes does not improve I/O performance for the test system that we have been using in this post when all MPI I/O settings are kept their default values. Increasing the number of MPI aggregators to 64 again yields a slightly better performance.


# Conclusions

In this post, we have taken a more closer look at collective MPI I/O and how its performance can be tuned via environment variables. There are a number of other fine tuning options that I haven't covered in this post,  the links I have provided shed more light on the matter. We also discussed how SSDs are finally becoming integrated in supercomputing systems focusing on Cray's Burst Buffer implementation. A realistic simulation example, which was perhaps slightly exaggerated in size, illustrated that significant time savings can be achieved in I/O intensive workflows by using the SSDs to buffer the data.